...
- `src/builtin.zig`: reusable built-in components and hooks such as `ProfilingMetrics`, `useScrolling` and the `ScrollBar` component.
...
```text
forbear/
├── CLAUDE.md               # Main entrypoint for agents
├── TODO.md
├── build.zig
├── build.zig.zon
├── playground.zig
