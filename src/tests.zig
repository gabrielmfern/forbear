...
/// A stub window for tests. The only field `frame()` touches is the event
/// queue, which it drains each frame; everything else stays undefined since
/// tests never reach the platform/cursor code paths. Reset to empty on every
/// `initTest` so events don't leak across tests.
var testWindow: forbear.Window = undefined;

fn initTest(allocator: std.mem.Allocator) !void {
...
test "overflow visible does not clip overflowing children" {
    ...
    try forbear.frame(try frameMeta(arena))({
        // Same geometry as the clipping test above, but the parent opts out
        // with overflow: visible → the overflowing child gets no clipRect.
        forbear.element(.{
            .style = .{
                .width = .{ .fixed = 100 },
                .height = .{ .fixed = 50 },
        ...
        for (cmds) |cmd| {
            try std.testing.expect(cmd.clipRect == null);
        }
    });
}
...
test "context value is visible to a descendant via useContext" {
    ...
    try forbear.frame(try frameMeta(arena))({
        if (SimpleCtx.useOrNull()) |v| observedAtRoot = v.*;
        forbear.element(.{
            ...
            SimpleCtx.Provider(@as(u32, 42))({
                forbear.element(.{
                    ...
                    if (SimpleCtx.useOrNull()) |v| observed = v.*;
                });
            });
        });
        _ = try forbear.layout();
    ...
    try forbear.frame(try frameMeta(arena))({
        NestedCtx.Provider(@as(u32, 1))({
            forbear.element(.{
                ...
                if (NestedCtx.useOrNull()) |v| outerObserved = v.*;
                NestedCtx.Provider(@as(u32, 2))({
                    forbear.element(.{
                        .style = .{ .width = .{ .fixed = 50.0 }, .height = .{ .fixed = 50.0 } },
                    })({
                        if (NestedCtx.useOrNull()) |v| middleObserved = v.*;
                        NestedCtx.Provider(@as(u32, 3))({
                            forbear.element(.{
                                .style = .{ .width = .{ .fixed = 10.0 }, .height = .{ .fixed = 10.0 } },
                            })({
                                if (NestedCtx.useOrNull()) |v| innermostObserved = v.*;
                            });
                        });
                    });
                });
        ...
        SimpleCtx.Provider(@as(u32, 100))({
            ThemeCtx.Provider(@as(u32, 200))({
                forbear.element(.{
                    ...
                    if (SimpleCtx.useOrNull()) |v| observedSimple = v.*;
                    if (ThemeCtx.useOrNull()) |v| observedTheme = v.*;
                });
            });
        });
        _ = try forbear.layout();
...
test "useContext returns null when no provider has been mounted" {
    ...
    try forbear.frame(try frameMeta(arena))({
        if (SimpleCtx.useOrNull()) |v| atTopLevel = v.* else atTopLevel = null;
        forbear.element(.{
            .style = .{ .width = .{ .fixed = 50.0 }, .height = .{ .fixed = 50.0 } },
        })({
            forbear.element(.{
                ...
                if (SimpleCtx.useOrNull()) |v| insideNestedElement = v.* else insideNestedElement = null;
            });
        });
        _ = try forbear.layout();
    });
...
test "provider value does not leak to siblings after its block ends" {
    ...
    try forbear.frame(try frameMeta(arena))({
        SiblingCtx.Provider(@as(u32, 7))({
            if (SiblingCtx.useOrNull()) |v| insideFirst = v.*;
        });
        if (SiblingCtx.useOrNull()) |v| betweenProviders = v.* else betweenProviders = null;
        SiblingCtx.Provider(@as(u32, 9))({
            if (SiblingCtx.useOrNull()) |v| insideSecond = v.*;
        });
        if (SiblingCtx.useOrNull()) |v| afterSecond = v.* else afterSecond = null;
        _ = try forbear.layout();
    });
    ...
    try forbear.frame(try frameMeta(arena))({
        StructCtx.Provider(initial)({
            forbear.element(.{
                ...
                if (StructCtx.useOrNull()) |v| observed = v.*;
            });
        });
        _ = try forbear.layout();
    });
        ...
        forbear.element(.{})({
            SlottedProvider()({
                forbear.element(.{
                    ...
                    if (SlotCtx.useOrNull()) |v| observed = v.*;
                });
            });
        });
        _ = try forbear.layout();
...
test "FocusContext: stale focus is dropped when widget does not re-register" {
    ...
    // resolve should drop the stale focus.
