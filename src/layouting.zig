...
/// When set, `updateFittingForAncestorsInDirection` writes a trace of each
/// ancestor visited, the decision made, values before/after, and why
/// propagation stopped. Leave `null` (the default) for zero overhead.
    ...
    while (ancestorIndexOptional) |ancestorIndex| {
        ...
        if (ancestor.style.getPreferredSize(direction) == .fit) {
            if (ancestor.style.direction == direction) {
                ...
                if (ancestorWraps) {
                    if (trace_writer) |tw| {
                        tw.writeAll("    STOP: wrapping boundary\n") catch {};
                    }
                    break;
                }
            }
            ...
            if (trace_writer) |tw| {
                tw.writeAll("    STOP: non-fit ancestor\n") catch {};
            }
            break;
        }
