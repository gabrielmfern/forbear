...
- The checked-in environment expects Linux rendering to work through Wayland. It keeps a stable fallback socket at `/tmp/cursor-wayland/wayland-0`.
- On startup, the environment first tries to reuse an already-running Wayland compositor by linking any existing `/run/user/*/wayland-*` socket into that fallback path. If no compositor is available, it starts `weston` instead.
