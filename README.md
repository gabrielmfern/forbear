...
Right now, the most picked option for new desktop apps are basically Electron. And there has been a growing negative sentiment, from myself as well, towards Electron. Electron was great, anyone that says otherwise is lying to themselves, but it feels like it's time for a change. Apps easily use loads of RAM, take lots of CPU, and even though it is possible to get performance to an acceptable point, I don't think it's possible to get it to feel amazing.

Something better, that serves as a better foundation is the way for us all to have great apps all around. I don't think we can really rely on the overall industry doing this, because it just doesn't pay off enough, even though it does an amount. If it did, we would already have a better option.
...
This is a multi-year project, so my planning might change and that's actually very likely, so I don't want to tie myself to the priorities of today.
...
[Flutter](https://github.com/flutter/flutter): Requires Dart and its runtime, which means a GC, a VM, and a heavier dependency chain than I want. In an ideal world I want to pick any language and still have everything work basically the same with the least amount of downsides.
