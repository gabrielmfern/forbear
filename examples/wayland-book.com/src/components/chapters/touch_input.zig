...
pub fn TouchInput() void {
    forbear.component(.{})({
        ...
        Paragraph(.{})({
            forbear.text("The \"x\" and \"y\" coordinates are fixed-point coordinates in the coordinate space of the surface which was touched — which is given in the \"surface\" argument. The time is a monotonically increasing timestamp with an arbitrary epoch, in milliseconds. Note also the inclusion of a serial, which can be included in future requests to associate them with this input event.");
        });
