...
pub fn WhatsInThePackage() void {
    forbear.component(.{})({
        ...
        Paragraph(.{})({
            forbear.text("When you install \"wayland\" in your Linux distribution, you'll most likely end up with the freedesktop.org distribution of libwayland-client, libwayland-server, wayland-scanner, and wayland.xml. Respectively, these will probably be in /usr/lib & /usr/include, /usr/bin, and /usr/share/wayland/. This package represents the most popular implementation of the Wayland protocol, but it is not the only one. Chapter 3 covers this implementation of Wayland in detail; the rest of the book is equally applicable to any implementation.");
        });
        ...
        Paragraph(.{})({
            forbear.text("Wayland protocols are defined by XML files. If you locate and open \"wayland.xml\" in your favorite text editor, you will find the XML specification for the \"core\" Wayland protocol. This is a high-level protocol — built on top of the wire protocol that we'll discuss in the next chapter. Most of this book is devoted to explaining this file.");
        });
